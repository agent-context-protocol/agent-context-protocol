    ...
    def _fetch_page(self, url: str) -> None:
        ...
        try:
            if url.startswith("file://"):
                ...
                download_path = os.path.normcase(os.path.normpath(unquote(url[5:])))
                res = self._mdconvert.convert_local(download_path)
                self.page_title = res.title
                self._set_page_content(res.text_content)
            else:
                ...
                ##############
                done_request_bool = True
                while done_request_bool:
                    s=time.time()

                    try: 
                        response = requests.get(url, timeout=30, **request_kwargs)
                        done_request_bool = False
                    except requests.exceptions.Timeout as err: 
                        print("Timeout err: ",err)
                        response = None
                        raise requests.exceptions.RequestException("An error occurred with the request.")
